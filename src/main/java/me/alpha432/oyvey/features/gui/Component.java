...
import me.alpha432.oyvey.util.Animation;
...
public class Component
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        int color = ColorUtil.toARGB(ClickGui.getInstance().catcolor.getValue().getRed(), ClickGui.getInstance().catcolor.getValue().getGreen(), ClickGui.getInstance().catcolor.getValue().getBlue(), 255);
        ...
        if (this.open) {
            if (ClickGui.getInstance().outline.getValue()) drawOutline(color);
            RenderUtil.rect(context.getMatrices(), (float) this.x, (float) this.y + 12.5f, (float) (this.x + this.width), (float) ((float) ((this.y + this.height) + totalItemHeight) * animation.getEase()), new Color(0,0,0,ClickGui.getInstance().alpha1.getValue()).getRGB());
        }
        if (ClickGui.getInstance().catcount.getValue()) drawString("[" + this.getItems().size() + "]", this.x - 1.0f + (float) this.width - mc.textRenderer.getWidth("[" + this.getItems().size() + "]") ,this.y + 2.5,-1);
        drawString(this.getName(), (float) this.x + 3.0f, (float) this.y - 4.0f - (float) OyVeyGui.getClickGui().getTextOffset(), -1);
        //OyVey.fontManager.drawText(context,this.getName(), (int) ((float) this.x + 1), (int) ((float) this.y - 4.0f - (float) OyVeyGui.getClickGui().getTextOffset()),new Color(-1));
        if (this.open) {
